...
const MatrixBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const speeds1 = Array.from(
      { length: columns },
      () => Math.random() * (maxSpeed1 - minSpeed1) + minSpeed1
    )
    const speeds2 = Array.from(
      { length: columns },
      () => Math.random() * (maxSpeed2 - minSpeed2) + minSpeed2
    )
    const speeds3 = Array.from(
      { length: columns },
      () => Math.random() * (maxSpeed3 - minSpeed3) + minSpeed3
    )
    const speeds4 = Array.from(
      { length: columns },
      () => Math.random() * (maxSpeed4 - minSpeed4) + minSpeed4
    )
